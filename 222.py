# =============================================================================
# Weekly Report Workflow Agent v2.0
# =============================================================================
# 设计目标：
# 1. 通用性：不依赖固定日报模板，不维护复杂业务 parser
# 2. 适配弱模型：大输入先拆 unit，再自动打包 chunk，避免一次读完整周
# 3. 降低错分：每个文本单元带 unit_id，模型只需引用 unit_id，程序侧强制回填人/日期/mention
#
# 核心流程：
# Step 1: 收集日报 + 通用展平
...
def build_indexer_units(users_data):
    ...
    后续模型输出只要带 unit_id，程序就能强制回填人/日期，降低错分。
    """
    units = []
...
def split_oversized_unit(unit, max_chars_per_chunk):
    ...
    切分后的 part 仍然保留同一个 user/date/mention，但 unit_id 会带 part。
    """
...
def build_indexer_chunks(users_data, max_chars_per_chunk=10000):
    ...
    策略：
    1. 先构造 member-date unit
    2. 如果单个 unit 超长，则继续按段落切 part
    3. 多个 unit 自动打包成 chunk
    4. 每个 chunk 长度不超过 max_chars_per_chunk
...
# =============================================================================
# 7. Step 2.2 - Indexer Agent 分块抽取 skeleton
# =============================================================================
def extract_skeletons_from_chunk(chunk, project):
    ...
    强约束：
    - 每条 skeleton 必须输出 unit_id
    - date/user_label/user_mention 虽然要求输出，但最终不会完全信任模型
    - 程序会根据 unit_id 强制回填 date/user_label/user_mention
    """
...
"""
    ...
    for item in skeletons:
        ...
        # 如果模型没输出 unit_id，或者 unit_id 不属于当前 chunk，先保留但标记为 invalid
        # 后续 normalize 会过滤掉 invalid unit_id，防止来源错分
        normalized.append({
            "unit_id": unit_id,
...
def normalize_skeletons_with_unit_map(raw_skeletons, unit_map):
    ...
    - 不相信模型输出的人名/日期
    - 只相信 unit_id
    - unit_id 匹配成功后，用程序记录的 metadata 覆盖 date/user/mention

    如果 unit_id 无效，则丢弃该 skeleton，避免错分。
    """
    normalized = []
    dropped = []
    ...
    for item in raw_skeletons:
        ...
        # key_output 为空的 skeleton 价值不大，直接丢弃，避免后续污染
        if not key_output:
            dropped.append({
...
def semantic_cluster_skeletons(skeletons, project):
    ...
    注意：
    date/user/mention 已经由程序根据 unit_id 固定。
    Planner 不能修改来源，只能组织主题。
    """
    if not skeletons:
        return {"clusters": []}
    ...
    for cluster in clustered.get("clusters", []):
        ...
        for task in cluster.get("tasks", []):
            ...
            if unit_id in skeleton_by_unit:
                ...
                # 强制用 skeleton 的来源和事实字段覆盖，防止 Planner 改错
...
# =============================================================================
# 9. Step 2.4 - Writer Agent 生成周报正文
# =============================================================================
...
# =============================================================================
# 14. 主执行流程 Orchestrator
# =============================================================================
print("=" * 60)
print(f"开始执行周报 Workflow Agent v2.0 | 项目数: {len(projects)}")
print("=" * 60)
